...
-- ==============================
-- TABELA: META (MODIFICADA - sem id_familia)
-- ==============================
CREATE TABLE meta
(
    ...
    tipo               ENUM('INDIVIDUAL', 'COLETIVA', 'COMUNITARIA') DEFAULT 'INDIVIDUAL',
    criado_por_familia INT, -- Família que criou a meta (opcional)
    CHECK (data_fim > data_inicio),
    FOREIGN KEY (criado_por_familia) REFERENCES familia (id_familia)
);

-- ==============================
-- TABELA: FAMILIA_META (NOVA - Entidade Associativa M:N)
-- ==============================
CREATE TABLE familia_meta
(
    id_familia           INT  NOT NULL,
    id_meta              INT  NOT NULL,
    data_associacao      DATE NOT NULL DEFAULT (CURRENT_DATE),
    progresso_individual DECIMAL(5, 2) DEFAULT 0.00,
    ativa                BOOLEAN       DEFAULT TRUE,
    observacoes          VARCHAR(255),

    PRIMARY KEY (id_familia, id_meta),
    ...
    CHECK (progresso_individual >= 0 AND progresso_individual <= 100)
);
...
-- Novos índices para familia_meta
CREATE INDEX idx_familia_meta_familia ON familia_meta (id_familia);
CREATE INDEX idx_familia_meta_meta ON familia_meta (id_meta);
CREATE INDEX idx_familia_meta_ativa ON familia_meta (ativa);
CREATE INDEX idx_meta_tipo ON meta (tipo);
